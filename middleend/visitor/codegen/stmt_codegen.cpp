        ...
        if (node.params) {
            ...
            for (auto* param : *node.params) {
                ...
                if (paramType == DataType::PTR || paramType == DataType::F32_PTR) {
                    ...
                    // Store parameter attributes for array indexing
                    // For pointer parameters, only store element type, not array dims
                    // (pointer params use scalar GEP, not array GEP)
                    FE::AST::VarAttr attr;
                    attr.type = param->type;
                    // Don't store arrayDims for pointer parameters
