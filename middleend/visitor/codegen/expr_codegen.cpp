...
namespace ME
{
    void ASTCodeGen::visit(FE::AST::LeftValExpr& node, Module* m)
    {
        ...
        if (reg != static_cast<size_t>(-1)) isPtrParam = paramPtrTab[reg];
            ...
            if (node.indices && !node.indices->empty()) {
                ...
                if (isPtrParam) {
                    ...
                    for (auto* index : *node.indices) {
                        ...
                        idxRegs.push_back(idxReg);
                    }
                    size_t offsetReg = linearizeIndices(dims, idxRegs);
                    size_t resReg    = getNewRegId();
                    std::vector<Operand*> gepIdx = {getRegOperand(offsetReg)};
                    insert(createGEP_I32Inst(elemType, base, {}, gepIdx, resReg));
                    addrOp = getRegOperand(resReg);
                } else {
                    // Regular array on stack or global
        ...
        if (type == DataType::PTR && numIndices == 0) {
            ...
            if (reg != static_cast<size_t>(-1)) {
                if (isPtrParam) {
                    ...
                    auto zeroIdx = buildZeroIndexList(dims.size() + 1);
                    insert(createGEP_I32Inst(elemType, addrOp, dims, zeroIdx, resReg));
                }
            } else {
                // Global array
                resReg = getNewRegId();
            ...
            auto zeroIdx = buildZeroIndexList(remainingDims.size() + 1);
            insert(createGEP_I32Inst(elemType, addrOp, remainingDims, zeroIdx, resReg));
        }
    ...
    void ASTCodeGen::handleAssign(FE::AST::LeftValExpr& lhs, FE::AST::ExprNode& rhs, Module* m)
    {
        ...
        bool     isPtrParam = (reg != static_cast<size_t>(-1) && paramPtrTab[reg]);
        ...
        if (reg != static_cast<size_t>(-1)) {
            ...
            if (lhs.indices && !lhs.indices->empty()) {
                if (isPtrParam) {
                    // Pointer parameter LHS (same flattening logic as in visit(LeftValExpr))
                    std::vector<int> dims;
                    if (reg2attr.find(reg) != reg2attr.end()) {
                        dims = reg2attr[reg].arrayDims;
                    }
                    ...
                    insert(createGEP_I32Inst(elemType, base, {}, {getRegOperand(offsetReg)}, resReg));
                    addrOp = getRegOperand(resReg);
                } else {
                    std::vector<Operand*> idxOps;
                    idxOps.push_back(getImmeI32Operand(0));
        ...
        FE::AST::BinaryExpr& node, FE::AST::ExprNode& lhs, FE::AST::ExprNode& rhs, Module* m)
    {
        ...
        auto inferActualType = [this](DataType annotatedType) {
            DataType actualType = annotatedType;
            if (!curBlock->insts.empty()) {
                Instruction* lastInst = curBlock->insts.back();
                if (auto* loadInst = dynamic_cast<LoadInst*>(lastInst)) actualType = loadInst->dt;
                else if (auto* arithInst = dynamic_cast<ArithmeticInst*>(lastInst)) actualType = arithInst->dt;
                else if (dynamic_cast<IcmpInst*>(lastInst) || dynamic_cast<FcmpInst*>(lastInst)) actualType = DataType::I1;
            }
            return actualType;
        };
        DataType lhsType = inferActualType(convert(lhs.attr.val.value.type));
        ...
        dispatch(&rhs, m);
        size_t rhsReg = getMaxReg();
        DataType rhsType = inferActualType(convert(rhs.attr.val.value.type));
        ...
        if (node.args) {
            for (size_t i = 0; i < node.args->size(); ++i) {
                ...
                if (i < argTypes.size()) {
                    ...
                    if (!compatible) {
                        // Skip type conversion for pointer types (PTR, F32_PTR)
                        // They should be handled separately or are already compatible
                        if (argType != DataType::PTR && argType != DataType::F32_PTR &&
                            expected != DataType::PTR && expected != DataType::F32_PTR) {
                            std::cerr << "[debug] converting arg for " << name << " from "
                                      << static_cast<int>(argType) << " to " << static_cast<int>(expected) << std::endl;
                            auto insts = createTypeConvertInst(argType, expected, argReg);
                            for (auto* inst : insts) insert(inst);
                            argReg = getMaxReg();
                            argType = expected;
                        } else {
                            // For pointer types, just update argType without conversion
                            argType = expected;
                        }
                    }
                }
