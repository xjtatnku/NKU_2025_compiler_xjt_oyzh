...
{ /*
   ...
   */
     ...
     * SysY类型转换规则：
     * 1. 算术运算类型提升：
     *    - float + int   -> float  (浮点优先级最高)
     *    - int + bool    -> int    (bool转为int：true=1, false=0)
     *    - float + bool  -> float
     * 2. 条件表达式：
     *    - int/float/bool 都可以用于条件判断
     *    - 非零值视为true，零值视为false
     * 3. 关系运算符：始终返回bool类型
     * 4. void类型不参与任何运算
     */
    ...
    {
        // Check for void type operands (e.g., void function return in expression)
        if (lhs.value.type->getBaseType() == Type_t::VOID) {
            errors.push_back("Left operand cannot be void at line " + std::to_string(node.line_num));
            hasError = true;
            ExprValue result;
            result.value.type = voidType;
            result.isConstexpr = false;
        ...
        if (rhs.value.type->getBaseType() == Type_t::VOID) {
            errors.push_back("Right operand cannot be void at line " + std::to_string(node.line_num));
            hasError = true;
            ExprValue result;
            result.value.type = voidType;
            result.isConstexpr = false;
